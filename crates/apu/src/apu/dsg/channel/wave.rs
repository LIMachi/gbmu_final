...
impl SoundChannel for Channel {
    ...
    fn on_enable(&mut self, io: &mut IORegs) {
        for pr in PATTERN_REG {
            io.io(pr).set_access(AccessMode::unused());
        }
    }

    fn on_disable(&mut self, io: &mut IORegs) {
        for pr in PATTERN_REG {
            io.io(pr).set_access(AccessMode::rw());
        }
    }

    fn power_on(&mut self, io: &mut IORegs) {
        io.io(IO::NR30).set_access(IO::NR30.access());
        for pr in PATTERN_REG {
            io.io(pr).set_access(AccessMode::rw());
        }
    }
    ...
    fn power_off(&mut self, io: &mut IORegs) {
        io.io(IO::NR30).direct_write(0).set_access(AccessMode::rdonly());
        for pr in PATTERN_REG {
            io.io(pr).direct_write(0).set_access(AccessMode::rdonly());
        }
    }
}
